...
const { GoveeError } = require('./govee-client');
...
const ALLOWED_ACTIONS = new Set([
  ...
  'list_light_presets',
  'save_light_preset',
  'activate_light_preset',
  'claim_oldest_codex_task',
  'get_codex_task_brief',
  'record_codex_progress',
  'mark_codex_waiting',
  'complete_codex_delegation',
  'fail_codex_delegation',
  'release_expired_codex_claims',
  'requeue_codex_task',
  'get_codex_delegation_status',
]);
...
function createPrivateIpcServer({
  getDatabase,
  taskService,
  homeService,
  delegationService,
  ...
  for (const method of ['listLights', 'listScenes', 'controlLights', 'activateScene', 'listPresets', 'savePreset', 'activatePreset']) {
    if (typeof homeService?.[method] !== 'function') throw new TypeError(`homeService.${method} is required`);
  }
  for (const method of ['claimOldest', 'getBrief', 'progress', 'waitForInput', 'complete', 'fail', 'releaseExpired', 'requeue', 'status']) {
    if (typeof delegationService?.[method] !== 'function') throw new TypeError(`delegationService.${method} is required`);
  }
  ...
  async function dispatch(action, params) {
    ...
    if (action === 'list_lights') return homeService.listLights(params);
    if (action === 'list_light_scenes') return homeService.listScenes(params);
    if (action === 'control_lights') return homeService.controlLights(params);
    if (action === 'activate_light_scene') return homeService.activateScene(params);
    if (action === 'list_light_presets') return homeService.listPresets(params);
    if (action === 'save_light_preset') return homeService.savePreset(params);
    if (action === 'activate_light_preset') return homeService.activatePreset(params);
    if (action === 'claim_oldest_codex_task') return delegationService.claimOldest(params);
    if (action === 'get_codex_task_brief') return delegationService.getBrief(params);
    if (action === 'record_codex_progress') return delegationService.progress(params);
    if (action === 'mark_codex_waiting') return delegationService.waitForInput(params);
    if (action === 'complete_codex_delegation') return delegationService.complete(params);
    if (action === 'fail_codex_delegation') return delegationService.fail(params);
    if (action === 'release_expired_codex_claims') return delegationService.releaseExpired(params);
    if (action === 'requeue_codex_task') return delegationService.requeue(params);
    if (action === 'get_codex_delegation_status') return delegationService.status(params);
  ...
  function handleConnection(socket) {
    ...
    socket.setTimeout(connectionReadTimeoutMs, () => socket.destroy());
    socket.on('data', async (chunk) => {
      ...
      if (newline === -1) return;
      // The short timeout protects the credential-bearing request read. Once a
      // complete request is present, external service convergence may safely
      // take longer without truncating the eventual tool receipt.
      socket.setTimeout(0);
      socket.pause();
      ...
      try {
        ...
        if (error instanceof TaskServiceError || error instanceof GoveeError) {
