...
function createVoiceCommandService({
  ...
  async function resolveNamedPresetIntent(text) {
    const match = /^(?:activate|use|set)(?: the)?(?: lights? to)?(?: preset)?\s+(.+?)(?:\s+preset)?$/i.exec(text);
    if (!match || typeof homeService.listPresets !== 'function') return null;
    try {
      const listed = await homeService.listPresets();
      const query = normalizeWords(match[1]);
      const matches = (listed?.presets || []).filter((preset) => normalizeWords(preset.name) === query);
      return matches.length === 1 ? baseIntent('activate_light_preset', { presetName: matches[0].name }) : null;
    } catch {
      return null;
    }
  }
  ...
  async function execute(input) {
    ...
    try {
      const namedPreset = await resolveNamedPresetIntent(text);
      const classified = !namedPreset && classifyIntent ? await classifyIntent({ text, now: instant.toISOString(), timeZone }) : null;
      const intent = namedPreset || classified || deterministicIntent(text, instant, timeZone);
