...
function cleanIdentifier(value, maximum, name) {
  if (typeof value !== 'string') fail('invalid_request', `${name} must be a string`);
  const result = value.trim();
  if (!result) fail('invalid_request', `${name} is required`);
  if (Buffer.byteLength(result) > maximum) fail('invalid_request', `${name} is too long`);
  if (!/^[A-Za-z0-9][A-Za-z0-9._:-]*$/.test(result)) fail('invalid_request', `${name} is invalid`);
  return result;
}
...
function migrateDelegationSchema(database, { now = new Date().toISOString(), idFactory = () => crypto.randomUUID() } = {}) {
  ...
  for (const record of database.taskDelegations) {
    ...
    const reopenedAtMs = Date.parse(task.reopenedAt || '');
    const completedAtMs = Date.parse(record.completedAt || record.updatedAt || record.createdAt || '');
    if (
      !task.done &&
      record.state === 'completed' &&
      Number.isFinite(reopenedAtMs) &&
      Number.isFinite(completedAtMs) &&
      reopenedAtMs >= completedAtMs
    ) {
      record.state = 'ready';
...
function createDelegationService({
  ...
  function getBrief(input) {
    ...
    return commit((database) => {
      const { record, task } = recordAndTask(database, cleanIdentifier(input.taskId, 128, 'task ID'));
  ...
  function releaseExpired(input = {}) {
    ...
    const requestedTaskId = input.taskId === undefined ? null : cleanIdentifier(input.taskId, 128, 'task ID');
    const instant = at();
    return commit((database) => {
      const released = [];
      for (const record of database.taskDelegations) {
  ...
  function requeue(input) {
    ...
    const taskId = cleanIdentifier(input.taskId, 128, 'task ID');
    const timestamp = at().toISOString();
    return commit((database) => {
