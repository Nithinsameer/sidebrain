...
function isLight(device) {
  if (LIGHT_TYPES.has(device?.type)) return true;
  return (device?.capabilities || []).some((capability) => USEFUL.has(capabilityKey(capability)));
}
...
function createHomeService({ getDatabase, replaceDatabase, persistDatabase, goveeClient, now = () => new Date() } = {}) {
  ...
  async function controlLights(input) {
    ...
    return { changed: devices.map(publicDevice), settings: { ...settings, rgb: input.settings.rgb } };
  }
  async function activateScene(input) {
    ...
    for (const { device, scene } of plan) {
      ...
      activated.push({ light: publicDevice(device), scene: { id: scene.id, name: scene.name, kind: scene.kind } });
    }
    return { activated };
  }
  ...
  async function activatePreset(input) {
    ...
    return { preset: { id: matches[0].id, name: matches[0].name }, changed };
  }
