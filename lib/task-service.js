...
function normalizeCreateReminderTask(input) {
  assertObject(input, 'request');
  checkRequestSize(input);
  assertAllowedKeys(input, new Set([
    'idempotency_key',
  ...
  const reminderAt = boundedString(input.reminder_at, 'reminder_at', 16, { required: true });
  const match = LOCAL_MINUTE_RE.exec(reminderAt);
  if (!match) fail('invalid_request', 'reminder_at must use YYYY-MM-DDTHH:MM local time');
  const timeZone = boundedString(input.timezone, 'timezone', 64, { required: true });
  if (!validTimeZone(timeZone)) fail('invalid_request', 'timezone must be a valid IANA timezone');

  return {
    idempotencyKey: normalizeIdempotencyKey(input.idempotency_key),
    origin: normalizeOrigin(input.origin),
    title: boundedString(input.title, 'title', 200, { required: true }),
    reminderAt: normalizeLocalMoment({
      date: match[1],
      time: match[2],
      timeZone,
    }, 'reminder_at'),
...
function normalizeCodexMark(input) {
  assertObject(input, 'request');
  checkRequestSize(input);
  assertAllowedKeys(input, new Set(['idempotencyKey', 'origin', 'taskId', 'projectAlias']), 'request');
  const projectAlias = boundedString(input.projectAlias || 'mindchuck', 'projectAlias', 40, { required: true }).toLowerCase();
  if (projectAlias !== 'mindchuck') fail('invalid_request', 'projectAlias is not approved');
  return {
    idempotencyKey: normalizeIdempotencyKey(input.idempotencyKey),
    origin: normalizeOrigin(input.origin),
    taskId: boundedString(input.taskId, 'taskId', 128, { required: true }),
...
function createTaskService({
  ...
  function cancelActiveReminders(database, {
    taskId,
    cancelledAtUtc,
    reason,
    operationId = null,
    ...
    if (!CANCELLATION_REASONS.has(reason)) throw new Error('invalid reminder cancellation reason');
    let cancelled = 0;
    for (const reminder of database.reminders) {
      if (reminder.taskId !== taskId || reminder.channel !== 'discord' || !ACTIVE_REMINDER_STATES.has(reminder.state)) continue;
      reminder.state = 'cancelled';
      reminder.cancelledAtUtc = cancelledAtUtc;
      reminder.cancellationReason = reason;
      reminder.cancelledByOperationId = operationId;
      reminder.cancellationOrigin = ORIGINS.has(origin) ? origin : null;
  ...
  function createReminderTask(input) {
    const normalized = normalizeCreateReminderTask(input);
    const repeated = idempotentResult('create_reminder_task', normalized);
    if (repeated) return repeated;

    const createdAtDate = currentNow();
    if (Date.parse(normalized.reminderAt.atUtc) <= createdAtDate.getTime()) {
      fail('invalid_request', 'reminder_at must be in the future');
    }
    ...
    return commit((database) => {
      const operation = {
        ...
        idempotencyKey: normalized.idempotencyKey,
        payloadHash: payloadHash('create_reminder_task', normalized),
        origin: normalized.origin,
        taskId: idFactory(),
        createdAt,
      };
      const task = {
        ...
        operationId: operation.id,
        pinned: false,
        tagIds: tagIdsFor(database, ['Reminder'], createdAt, idFactory),
        files: [],
        list: false,
        checked: [],
        task: true,
        ...
        completedAt: null,
        plannedFor: normalized.reminderAt.date,
        dueTime: normalized.reminderAt.time,
        dueTimeZone: normalized.reminderAt.timeZone,
        dueAtUtc: normalized.reminderAt.atUtc,
        taskNotified: true,
        followUpDate: null,
        parentId: null,
        canvas: { on: false, x: 40, y: 40 },
  ...
  function setTaskCompletion(input) {
    ...
    return commit((database) => {
      ...
      const statusChanged = wasCompleted !== normalized.completed;
      const operation = {
        id: idFactory(),
        ...
        taskId: task.id,
        createdAt: changedAt,
        requestedCompleted: normalized.completed,
        resultingCompleted: normalized.completed,
        statusChanged,
        cancelPendingRemindersRequested: normalized.cancelUndeliveredReminders,
      ...
      if (statusChanged) {
        ...
        if (normalized.completed) {
          ...
          operation.cancelledReminderCount = normalized.cancelUndeliveredReminders
            ? cancelActiveReminders(database, {
              taskId: task.id,
              cancelledAtUtc: changedAt,
  ...
  function markTaskForCodex(input) {
    const normalized = normalizeCodexMark(input);
    const repeated = idempotentResult('mark_task_for_codex', normalized);
    if (repeated) return repeated;
    const changedAt = currentNow().toISOString();
    return commit((database) => {
      const task = database.messages.find((message) => message.id === normalized.taskId && message.task && !message.deletedAt);
      if (!task) fail('not_found', 'task was not found');
      if (task.done) fail('invalid_state', 'a completed task cannot be delegated');
      const operation = {
        id: idFactory(), type: 'mark_task_for_codex', idempotencyKey: normalized.idempotencyKey,
        payloadHash: payloadHash('mark_task_for_codex', normalized), origin: normalized.origin,
        taskId: task.id, createdAt: changedAt,
      };
      task.tagIds = [...new Set([...(task.tagIds || []), ...tagIdsFor(database, ['codex', `project:${normalized.projectAlias}`], changedAt, idFactory)])];
      task.updatedAt = changedAt;
      database.taskOperations.push(operation);
      return buildReceipt(database, operation);
    });
  ...
  function getTaskReceipt(input) {
    ...
    if (query.taskId) {
      operation = database.taskOperations.find((candidate) =>
        candidate.taskId === query.taskId &&
        ['create_task', 'create_reminder_task'].includes(candidate.type)) ||
    ...
    for (const id of ids) {
      ...
      try {
        const notificationTitles = {
          delegation_completed: 'Sidebrain Codex completed',
          delegation_waiting: 'Sidebrain Codex needs input',
          delegation_failed: 'Sidebrain Codex failed',
        };
        const response = await deliverDiscord({
          reminderId: leased.id,
          taskId: leased.taskId,
          title: notificationTitles[leased.kind] || (leased.kind === 'follow_up' ? 'Sidebrain follow-up' : 'Sidebrain reminder'),
