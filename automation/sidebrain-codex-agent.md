...
2. Call `release_expired_codex_claims` once for recovery. Then call `claim_oldest_codex_task` at most once in this run. If another delegation is active or no task is eligible, exit silently and successfully.
...
9. Never process a waiting, completed, failed, cancelled, or currently claimed task as new work. Never send messages, submit forms, make purchases, delete data, or modify external accounts unless the individual task allows the exact action and Sameer separately approves it. Keep detailed results in Sidebrain, not Discord.
